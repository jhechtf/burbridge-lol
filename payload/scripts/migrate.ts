...
function buildConfig(): Config {
	...
	const strapiUrl = cliArgs.strapiUrl ?? process.env.STRAPI_URL ?? "";
	const strapiToken = cliArgs.strapiToken ?? process.env.STRAPI_TOKEN ?? "";
	const payloadUrl =
		cliArgs.payloadUrl ?? process.env.PAYLOAD_URL ?? "http://localhost:3000";
	const payloadEmail = cliArgs.payloadEmail ?? process.env.PAYLOAD_EMAIL ?? "";
...
type StrapiItem<T> = { id: number; documentId?: string } & T;
...
async function payloadFindBySlug(
	payloadUrl: string,
	token: string,
	collection: string,
	slug: string,
...
async function payloadCreate(
	...
	const url = `${payloadUrl}/api/${collection}`;
	...
	if (!res.ok) {
		const body = await res.text();
		throw new Error(`Payload POST /${collection} HTTP ${res.status}: ${body}`);
	}
...
async function migrateTags(
	...
	for (const item of items) {
		try {
			const result = await payloadCreate(cfg.payloadUrl, payloadToken, "tags", {
				name: item.name,
			...
			console.log(
				`[${label}] Inserted tag "${item.display}" (strapi: ${item.id} → payload: ${result.id})`,
			);
		} catch (err) {
			recordSkipped(label);
			console.error(
				`[${label}] Failed to insert tag "${item.display}" (strapi: ${item.id}):`,
...
async function migrateStories(
	...
	for (const item of items) {
		const existingId = await payloadFindBySlug(
			cfg.payloadUrl,
			payloadToken,
			"stories",
			item.slug,
		...
		if (existingId !== null) {
			...
			recordSkipped(label);
			console.log(
				`[${label}] Skipping story "${item.title}" — slug already exists (payload: ${existingId})`,
			);
			continue;
		}

		const strapiTagIds = getRelationIds(item.tags);
		...
		try {
			const result = await payloadCreate(
				...
				"stories",
				{
					title: item.title,
					slug: item.slug,
					description: plainTextToLexical(item.synopsis),
					tags: payloadTagIds,
				},
			);
			idMap.set(item.id, result.id);
			recordInserted(label);
			console.log(
				`[${label}] Inserted story "${item.title}" (strapi: ${item.id} → payload: ${result.id})`,
			);
		} catch (err) {
			recordSkipped(label);
			console.error(
				`[${label}] Failed to insert story "${item.title}" (strapi: ${item.id}):`,
				(err as Error).message,
			);
		}
	}
		...
		try {
			const result = await payloadCreate(
				...
				"blogs",
				{
					title: item.title,
					slug: item.slug,
					contents: convertStrapiBlocksToLexical(item.contents),
					tags: payloadTagIds,
			...
			recordInserted(label);
			console.log(
				`[${label}] Inserted blog "${item.title}" (strapi: ${item.id} → payload: ${result.id})`,
			);
		} catch (err) {
			recordSkipped(label);
			console.error(
				`[${label}] Failed to insert blog "${item.title}" (strapi: ${item.id}):`,
				(err as Error).message,
			);
		}
	}
...
async function migrateOthers(
	...
	for (const item of items) {
		...
		try {
			...
			recordInserted(label);
			console.log(
				`[${label}] Inserted other "${item.title}" (strapi: ${item.id} → payload: ${result.id})`,
			);
		} catch (err) {
			recordSkipped(label);
			console.error(
				`[${label}] Failed to insert other "${item.title}" (strapi: ${item.id}):`,
				(err as Error).message,
			);
		}
	}
...
async function migrateChapters(
	...
	for (const item of items) {
		...
		if (existingId !== null) {
			recordSkipped(label);
			console.log(
				`[${label}] Skipping chapter "${item.chapter_title}" — slug already exists (payload: ${existingId})`,
			);
			continue;
		}

		const strapiStoryIds = getRelationIds(item.story);
		...
		if (strapiStoryIds.length > 0 && payloadStoryId === undefined) {
			console.warn(
				`[${label}] Chapter "${item.chapter_title}" (strapi: ${item.id}) references story strapi:${strapiStoryIds[0]} which has no Payload mapping — story relation will be omitted`,
			);
		}
		...
		try {
			...
			recordInserted(label);
			console.log(
				`[${label}] Inserted chapter "${item.chapter_title}" (strapi: ${item.id} → payload: ${result.id})`,
			);
		} catch (err) {
			recordSkipped(label);
			console.error(
				`[${label}] Failed to insert chapter "${item.chapter_title}" (strapi: ${item.id}):`,
				(err as Error).message,
			);
		}
	}
